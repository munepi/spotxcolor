...
\subsubsection{Why \cs{color@} is fragile}

To understand this fragility, one must examine the internal representation of colors in \pkg{xcolor}.
When |\definecolor{Plum}{rgb}{.5,0,1}| is called, \pkg{xcolor} creates a macro |\\color@Plum| containing the \emph{internal representation}:
\begin{quote}
|\xcolor@|\marg{type-spec}\marg{driver-spec}\marg{n-model}\marg{n-color-spec}
\end{quote}
where \meta{type-spec} is empty or |named|, \meta{driver-spec} is a driver-dependent string, \meta{n-model} is the color model name, and \meta{n-color-spec} is the normalised color values.
The crucial point is the definition of |\xcolor@| itself:
\begin{quote}
\begin{verbatim}
\def\xcolor@#1#2#3#4{#2}   % returns only driver-spec
\end{verbatim}
\end{quote}
When |\\color@Plum| is expanded, \emph{only the driver-spec is returned}; the model and color values are discarded.
This is a deliberate design for backward compatibility with the \pkg{color} package, where |\\color@Plum| expanded directly to a driver string.

The driver-spec format differs completely between drivers:
\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ll}
\hline
Driver & \texttt{\textbackslash\textbackslash color@Plum} expands to \\
\hline
dvips   & |rgb 0.5 0 1| \\
pdftex  & |0.5 0 1 rg 0.5 0 1 RG| \\
\hline
\end{tabular}
\end{center}

This creates three structural barriers for custom color models:
\begin{enumerate}
\item Driver-spec opacity:
  Even if a custom model stores a Separation identifier in \meta{driver-spec},
  the downstream |\set@color| mechanism simply pushes this string verbatim.
  The \pkg{pdfcol}/driver layer (e.g., \texttt{dvipdfmx.def}) interprets it
  as numeric CMYK/RGB/Gray values.
  There is no protocol for passing ``this is a Separation color'' through this channel.

\item Type dispatch fragility:
  \pkg{xcolor}'s internal |\XC@type| inspects whether the first token of |\\color@|\meta{name}
  is |\xcolor@| to classify colors into types (0--4: undefined, unidentified, named, declared, undeclared).
  Any modification to the internal structure risks breaking this token-level dispatch.

\item Historical instability:
  Prior to \pkg{xcolor} v1.10, the internal representation used a different command
  (|\xcolor@|\meta{name} with a different syntax).
  The format has changed before, and the current structure is not a public API contract.
\end{enumerate}

\noindent
This is why approaches that inject custom color models into \pkg{xcolor}'s internals
are fragile across versions and driver combinations.
...
Covers: |\color|, |\textcolor|, \pkg{colortbl} row colors, \pkg{tcolorbox} elements.
...
\begin{itemize}
\item No dvipdfmx/XeTeX support: The package directly uses |\pdfobj| and |\pdfpageresources|, which are pdfTeX/LuaTeX primitives.
There is no |\special|-based fallback.

\item Manual page resource management: Users must call |\pagecolorspace{...}| and |\resetpagecolorspace| to manage the |/ColorSpace| dictionary, which is fragile in modern \LaTeX.

\item No pattern support: While the package provides fill/stroke PGF driver macros, uncolored patterns still use the default |[/Pattern /DeviceCMYK]| color space with CMYK values.

\item DeviceN support: Uniquely among these packages, \textsf{colorspace} supports |/DeviceN| color spaces for multi-ink combinations.
\end{itemize}

\subsection{\textsf{spotxcolor} --- Late-Stage Interception}

As described in Section~1.2 (``The Core Principle''), \pkg{spotxcolor} (2026) takes a fundamentally different approach: rather than modifying \pkg{xcolor}'s internal color model, it lets \pkg{xcolor} operate entirely in CMYK and intercepts the output at multiple stages.
Four interception layers (|\set@color|, |\set@page@color|, PGF driver macros, and |\pgfsys@setpatternuncolored|) cover all output paths; see Section~2 for details.

This approach is completely transparent to all packages that use \pkg{xcolor} or PGF for color management, requiring no special commands or manual resource management.
...
\begin{enumerate}
\item Separation CS registration (|\spotxcolor_create_pdf_obj:nnn|):
...
\item Bulk resource fallback (|\AtBeginDocument|, |\g_spotxcolor_resource_tl|):
...
\item Pattern CS creation (|\__spotxcolor_create_pattern_cs:|):
...
\item CMYK Pattern CS (|\AtBeginDocument|, |/pgfpcmyk|):
