...
% =====================================================================
\section{Compatibility with pdfmanagement (v1.5)}
% =====================================================================
...
It redefines PGF's |\pgfutil@addpdfresource@colorspaces| with its own parser (|\__pdfmanagement_patch_pgfcolorspaces:w|), which expects arguments in the form |/KEY VALUE| and scans for a |\q_stop| delimiter.

\pkg{spotxcolor} v1.4 passed arguments like |/DIC161s \l_tmpb_tl \space 0 R| where |\l_tmpb_tl| was an already-expanded object number.
...
Similarly, direct |\pdfpageresources| access (used as a fallback when PGF is not loaded) is explicitly forbidden when pdfmanagement is active.

\subsection{The v1.5 Solution: Centralized Resource Dispatch}

v1.5 introduces a boolean flag |\g__spotxcolor_pdfmanagement_bool|, detected at load time via |\IfPDFManagementActiveTF|, and rewires the centralized resource helper |\spotxcolor_add_colorspace_resource:nn|:
...
\bool_if:NTF \g__spotxcolor_pdfmanagement_bool
  ...
  { % traditional: \pgfutil@addpdfresource@colorspaces
...
Four code paths were updated to use this centralized dispatch:

\begin{enumerate}
\item \textbf{Separation CS registration} (|\spotxcolor_create_pdf_obj:nnn|): Called at |\definespotcolor| time.
    Each spot color's |/NAME objRef| entry is registered via |\spotxcolor_add_colorspace_resource:nn|.
...
\item \textbf{Pattern CS creation} (|\__spotxcolor_create_pattern_cs:|):
    ...
    Previously called |\pgfutil@addpdfresource@colorspaces| directly; now uses the centralized helper.

\item \textbf{CMYK Pattern CS} (|\AtBeginDocument|, |/pgfpcmyk|):
    The |/pgfpcmyk [/Pattern /DeviceCMYK]| registration previously used |\pgfutil@addpdfresource@colorspaces|; now uses the centralized helper via |\csname| catcode bridging.
\end{enumerate}
