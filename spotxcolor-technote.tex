...
\subsection{The v1.6 Solution: Centralized Resource Dispatch}

v1.6 introduces a boolean flag |\g__spotxcolor_pdfmanagement_bool|, detected at package load time by checking the internal boolean |\g__pdfmanagement_active_bool| directly (not via |\IfPDFManagementActiveTF|, which can trigger side effects), and rewires the centralized resource helper |\spotxcolor_add_colorspace_resource:nn|:
...
Note the use of the |nnx| variant (not |nnn|): the third argument (object reference, e.g., |42 0 R|) must be x-expanded at call time.
