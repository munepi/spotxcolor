...
\begin{itemize}
\item Native \textsf{xcolor} Integration: Use spot colors exactly like standard colors (e.g., |\textcolor|, |\pagecolor|), generating perfect PDF structures for \texttt{pdftex} and \texttt{luatex}.
\item Universal Engine Support: All standard macros (|\color|, |\textcolor|, |\pagecolor|) produce true spot color output on all four engines (pdfTeX, LuaTeX, XeTeX, dvipdfmx) via automatic interception. No special commands or driver options are required.
\item Advanced Graphics (\textsf{TikZ}/\textsf{PGF}) Support: Internally patches \textsf{PGF} driver macros and pattern primitives to emit true spot color PDF operators.
...
\item Seamless Decorations: Works flawlessly with \textsf{colortbl} (zebra-striped tables) and \textsf{tcolorbox} (frames, backgrounds, shadows).
\item Complete Backward Compatibility: Perfectly emulates user commands from both \textsf{spotcolor} and \textsf{colorspace} packages.
\end{itemize}
...
\textcolor{PANTONE876C}{PANTONE 876 C (100\%)} \quad
\textcolor{PANTONE876C!50}{50\%} \quad
\textcolor{PANTONE876C!25}{25\%}
...
If you compile your document with \texttt{platex} or \texttt{uplatex} and generate a PDF via \texttt{dvipdfmx}, you \emph{must} specify the \texttt{dvipdfmx} driver option globally in your document class:
...
For the following examples, we have defined three spot colors forming a triadic color harmony: \textcolor{DIC161s}{DIC161s} (Vermilion), \textcolor{DIC256s}{DIC256s} (Green), and \textcolor{DIC200s}{DIC200s} (Purple).

\subsection{TikZ: Patterns and Multiply Blend Mode}

\textsf{spotxcolor} safely forces PGF's hardcoded RGB patterns into the CMYK color space to ensure print safety.
This CMYK conversion applies to all uncolored patterns, not just spot colors---so even in a pure CMYK workflow without any spot colors, loading \textsf{spotxcolor} eliminates the \texttt{/DeviceRGB} operators that PGF would otherwise emit for pattern fills.
It also flawlessly handles PDF blend modes (e.g., multiply) with spot colors.
...
This box is styled entirely using DIC 200s and its tints.
...
\vspace{0.5em}
\noindent You can also mix text colors easily: \textcolor{DIC161s}{Vermilion Text} and \textcolor{DIC256s}{Green Text}.
\end{tcolorbox}
...
Please do \emph{not} load the entire \texttt{dic.def} file (or any other large dictionary) into your preamble using \verb|\input|.
...
Instead, open \texttt{dic.def} in a text editor, copy \emph{only} the specific \verb|\definespotcolor| lines you actually need, and paste them into your document preamble.
\end{tcolorbox}
...
\textsf{spotxcolor} defines both of these as empty dummy macros (they do nothing).
Why?
...
For a detailed technical explanation of the interception architecture and why these limitations exist, see the companion document \texttt{spotxcolor-technote.pdf}.
...
See the Technical Note (\texttt{spotxcolor-technote.pdf}, ``Hatching vs.\ Shading: A Structural Difference'') for a detailed structural comparison.
...
\hline
|\color|, |\textcolor|     & \textcolor{DIC256s}{Spot} & All engines \\
|\pagecolor|               & \textcolor{DIC256s}{Spot} & All engines \\
|\SpotColor|               & \textcolor{DIC256s}{Spot} & Direct PDF literal \\
TikZ fill/stroke           & \textcolor{DIC256s}{Spot} & Via PGF driver patch \\
Low-level PGF commands     & \textcolor{DIC256s}{Spot} & Via PGF driver patch \\
Uncolored patterns         & \textcolor{DIC256s}{Spot} & Via \texttt{[/Pattern\space [/Separation~...]]} \\
Fading / transparency      & \textcolor{DIC256s}{Spot} & Fill color is spot \\
Opacity + blend modes      & \textcolor{DIC256s}{Spot} & Fill color is spot \\
\textsf{colortbl} row colors & \textcolor{DIC256s}{Spot} & Via |\set@color| patch \\
\textsf{tcolorbox}         & \textcolor{DIC256s}{Spot} & Frame, background, shadow \\
\hline
Proportional tints (\texttt{!N})   & \textcolor{DIC256s}{Spot} & Auto-detected \\
Non-proportional mixes (\texttt{!N!color}) & CMYK & Needs \texttt{/DeviceN} \\
Shading (gradients)        & RGB  & PGF hardcodes \texttt{/DeviceRGB} \\
\hline
\end{tabular}
\end{center}
...
It supports \texttt{pdftex} and \texttt{luatex}, but \emph{not} \texttt{dvipdfmx} or Xe\LaTeX.
