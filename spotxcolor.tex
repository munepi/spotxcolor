...
When pdfmanagement is active, it takes exclusive control of all PDF page resources (including the \texttt{/ColorSpace} dictionary), and direct access via |\pdfpageresources| or |\special{pdf:put}| is disabled.

This conflicts with the traditional approach used by \textsf{spotxcolor} (and by \textsf{PGF}'s |\pgfutil@addpdfresource@colorspaces|) to register Separation color space entries.

\subsection{How spotxcolor Handles This}

Starting with v1.5, \textsf{spotxcolor} detects pdfmanagement at load time using |\IfPDFManagementActiveTF| and automatically switches its resource registration strategy:

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ll}
\hline
\textbf{Mode} & \textbf{Resource Registration Method} \\
\hline
Traditional (no pdfmanagement) & |\pgfutil@addpdfresource@colorspaces| \\
                               & or direct |\pdfpageresources| / |\special{pdf:put}| \\
\hline
pdfmanagement active & |\pdfmanagement_add:nnn {Page} {ColorSpace}| \\
                     & (l3pdf API) \\
\hline
\end{tabular}
\end{center}
...
\subsection{Usage with \texttt{\textbackslash DocumentMetadata}}
...
\subsection{Known Interactions}

When pdfmanagement is active, PGF's own resource management also needs to cooperate with l3pdf.
Recent versions of \textsf{PGF} (3.1.10+) include built-in pdfmanagement awareness.
If you encounter warnings related to PGF resource registration, ensure your \TeX\ Live distribution is up to date.
